...
def _get_default_urdf_path() -> str:
    """返回可用的 reBot-DevArm URDF 文件路径，并确保包名符号链接存在。"""
    here = Path(__file__).resolve()
    candidate_bases = [
        here.parents[1] / "urdf",  # reBotArm_control_py/urdf
        here.parents[2] / "urdf",  # Servo_control/urdf, legacy layout
        here.parents[6] / "reBotArm_control_py" / "urdf",  # workspace-level control package
        Path.home() / "hjx_file" / "rebot_devarm_ws" / "reBotArm_control_py" / "urdf",
        Path.home() / "hjx_file" / "rebot_devarm_ws" / "reBotArm_develop_hjx" / "urdf",
    ]

    base = None
    for candidate in candidate_bases:
        urdf_file = candidate / "reBot-DevArm_fixend_description" / "urdf" / "reBot-DevArm_fixend.urdf"
        if urdf_file.exists():
            base = candidate
            break

    if base is None:
        searched = "\n  ".join(str(p) for p in candidate_bases)
        raise FileNotFoundError(f"找不到 reBot-DevArm 默认 URDF，已检查:\n  {searched}")
