...
class GripperCfg:
    ...
    pos_ki: float = 1.0
    vlim: float = 3.0
    position_open: float = -5.8
    position_close: float = 0.0
...
def load_cfg(path: str) -> dict:
    ...
    return {
        ...
        "gripper": GripperCfg(
            ...
            position_open=float(limits.get("position_open", -5.8)),
            position_close=float(limits.get("position_close", 0.0)),
            safety_margin=float(limits.get("safety_margin", 0.1)),
        ),
    }
...
class Gripper:
    ...
    def position_limits(self) -> tuple[float, float]:
        """返回按数值升序排列的软件位置限位。"""
        return tuple(sorted((self._cfg.position_open, self._cfg.position_close)))

    @property
    def safety_margin(self) -> float:
        return self._cfg.safety_margin

    @property
    def default_velocity_limit(self) -> float:
        return self._cfg.vlim

    def clamp_position(self, position: float, margin: float = 0.0) -> float:
        """将目标位置限制在机械行程内，可选保留两端安全余量。"""
        lower, upper = self.position_limits
        margin = max(0.0, float(margin))
        if lower + margin > upper - margin:
            raise ValueError("安全余量大于夹爪行程的一半")
        return min(upper - margin, max(lower + margin, float(position)))
