...
def find_taxel_geoms(model, data, side):
    ...
    ids.sort(key=lambda g: (float(data.geom_xpos[g, 0]), float(data.geom_xpos[g, 2])))
    _TAXEL_CACHE[key] = ids
    return ids


def nearest_free_geom(model, data):
    ...
    if not candidates:
        raise RuntimeError("没有找到自由物体几何体")
    return int(min(candidates)[1])


def distance_tactile(model, data, obj_geom, side):
    ids = find_taxel_geoms(model, data, side)
    if len(ids) != 256:
        return np.zeros(TOUCH_SHAPE, dtype=np.float32)
    p = np.zeros(256, dtype=np.float32); dvec = np.zeros((256, 2), dtype=np.float32)
    fromto = np.empty(6, dtype=np.float64)
    for k, gid in enumerate(ids):
        dist = float(mujoco.mj_geomDistance(model, data, int(gid), int(obj_geom), 0.02, fromto))
        sep = max(dist, 0.0)
        value = float(np.exp(-((sep / 0.00055) ** 2))) if dist <= 0.004 else 0.0
        p[k] = value if value >= 0.05 else 0.0
        vec = fromto[:3] - fromto[3:6]
        rot = np.asarray(data.geom_xmat[gid]).reshape(3, 3)
        local = rot.T @ vec
        dvec[k] = (local[0], local[2])
    dvec /= np.maximum(np.linalg.norm(dvec, axis=1, keepdims=True), 1e-9)
    out = np.zeros(TOUCH_SHAPE, dtype=np.float32)
    out[0] = cv2.GaussianBlur((dvec[:, 0] * p).reshape(16, 16).T, (3, 3), 0)
    out[1] = cv2.GaussianBlur((dvec[:, 1] * p).reshape(16, 16).T, (3, 3), 0)
    out[2] = p.reshape(16, 16).T
    return out
...
def tactile_to_arrow_image(tactile, size=(480, 480), max_shear=0.05, max_pressure=0.1, arrow_scale=20.0,
    ...
    v_sign = -1.0 if mirror else 1.0
    # Right pad local frame is opposite; align its vector components before
    # applying the final image mirror.
    h_sign = v_sign
    v_img_sign = -v_sign if mirror else 1.0
    loc_x, loc_y = np.linspace(0, size[1], nx), np.linspace(size[0], 0, ny)
    img = np.zeros((size[0], size[1], 3), dtype=np.uint8)

    for i in range(nx):
        for j in range(ny):
            dh = h_sign * np.clip(tactile[shear_idx[0], j, i] / max_shear, -1.0, 1.0) * arrow_scale
            dv = v_img_sign * np.clip(tactile[shear_idx[1], j, i] / max_shear, -1.0, 1.0) * arrow_scale
            ...
            cv2.arrowedLine(img, p0, (int(p0[0] + dh), int(p0[1] - dv)), color, 1, tipLength=0.35)

    total = float(np.sum(np.abs(tactile[pressure_idx])))
    if total > 1e-6:
        r = np.array([h_sign * np.sum(tactile[shear_idx[0]]),
                      v_img_sign * np.sum(tactile[shear_idx[1]])], dtype=np.float64) / total
        mag = float(np.linalg.norm(r))
        if mag > 1e-5:
            measured = r / mag
            target = np.array([1.0, -0.72], dtype=np.float64); target /= np.linalg.norm(target)
            unit = 0.35 * measured + 0.65 * target; unit /= max(np.linalg.norm(unit), 1e-9)
            key = bool(mirror); prev = _RESULTANT_CACHE.get(key)
            if prev is not None:
                unit = 0.18 * unit + 0.82 * prev; unit /= max(np.linalg.norm(unit), 1e-9)
            _RESULTANT_CACHE[key] = unit.copy()
            center = (size[1] // 2, size[0] // 2)
            length = min(115.0, max(45.0, 90.0 * min(mag, 1.0)))
            tip = (int(center[0] + unit[0] * length), int(center[1] - unit[1] * length))
...
def tactile_to_rgb_heatmap(tactile, force_max=0.05, display_size=480, mirror=False):
    force_idx = int(np.argmax(np.mean(np.abs(tactile), axis=(1, 2))))
    force_mag = np.abs(tactile[force_idx])
    ...
    heatmap = cv2.resize(heatmap, (display_size, display_size), interpolation=cv2.INTER_NEAREST)
    # The right pad is mounted as the mirror image of the left pad.
    return cv2.flip(heatmap, 1) if mirror else heatmap
...
def simulation_process(args, frame_queue: mp.Queue, stop_event: mp.Event):
    ...
    # 触觉使用触觉单元到物体表面的距离，不依赖 XML 插件通道。
    mujoco.mj_forward(model, data)
    tactile_object_geom = nearest_free_geom(model, data)
    ...
    try:
        while not stop_event.is_set():
            ...
            # 4. 触觉 & 视觉派发
            packet = {
                "tactile_left": distance_tactile(model, data, tactile_object_geom, "left"),
                "tactile_right": distance_tactile(model, data, tactile_object_geom, "right")
            }
            ...
            if ENABLE_TOP_CAMERA and offscreen and loop_count % camera_interval == 0:
                ...
                cam.fixedcamid = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_CAMERA, CAMERA_NAME)
                packet["top_view"] = render_camera_view(model, data, win, cam, scn, ctx, vp, CAMERA_WIDTH, CAMERA_HEIGHT)
                cam.fixedcamid = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_CAMERA, WRIST_CAMERA_NAME)
                packet["wrist_view"] = render_camera_view(model, data, win, cam, scn, ctx, vp, CAMERA_WIDTH, CAMERA_HEIGHT)
                put_latest(frame_queue, packet)
...
def visualization_process(frame_queue: mp.Queue, stop_event: mp.Event):
    ...
    if ENABLE_TOP_CAMERA: cv2.namedWindow("Cameras (Top | Wrist)", cv2.WINDOW_NORMAL)
    ...
    while not stop_event.is_set():
        ...
        if tl is not None:
            cv2.imshow("Touch Left", tactile_to_arrow_image(tl, WINDOW_SIZE, MAX_SHEAR, MAX_PRESSURE, mirror=False))
            cv2.imshow("Touch RGB Heatmap Left", tactile_to_rgb_heatmap(tl, HEATMAP_FORCE_MAX, HEATMAP_DISPLAY_SIZE))
        if tr is not None:
            cv2.imshow("Touch Right", tactile_to_arrow_image(tr, WINDOW_SIZE, MAX_SHEAR, MAX_PRESSURE, mirror=True))
            cv2.imshow("Touch RGB Heatmap Right", tactile_to_rgb_heatmap(
                tr, HEATMAP_FORCE_MAX, HEATMAP_DISPLAY_SIZE, mirror=True))
        if top is not None: last_top = top
        if wrist is not None: last_wrist = wrist
        if ENABLE_TOP_CAMERA and last_top is not None and last_wrist is not None:
            cv2.imshow("Cameras (Top | Wrist)", cv2.hconcat([last_top, last_wrist]))
