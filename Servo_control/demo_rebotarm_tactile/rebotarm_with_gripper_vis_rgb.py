        ...
        while not stop_event.is_set():
            ...
            if loop_count % vis_interval == 0:
                put_latest(frame_queue, {
                    "touch_right": read_tactile(right_reader, data, FORCE_MAX),
                    "touch_left": read_tactile(left_reader, data, FORCE_MAX)
                })
...
def visualization_process(frame_queue: mp.Queue, stop_event: mp.Event):
    ...
    while not stop_event.is_set():
        ...
        if tr is not None:
            cv2.imshow("Touch Heatmap - Sensor Right", tactile_to_colormap(tr, FORCE_MAX, HEATMAP_DISPLAY_SIZE))
        if tl is not None:
            cv2.imshow("Touch Heatmap - Sensor Left", tactile_to_colormap(tl, FORCE_MAX, HEATMAP_DISPLAY_SIZE))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vis-hz", type=float, default=50.0, help="OpenCV 热力图展示刷新率")
